  ...
  const clearAllParticles = useCallback(() => {
    ...
    particlesRef.current.forEach(particle => {
      ...
      gsap.to(particle, {
        ...
        ease: 'back.in(1.7)',
        onComplete: () => {
            if (particle && particle.parentNode && particle.parentNode.contains(particle)) {
              try {
...
const GlobalSpotlight: React.FC<{
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (spotlightRef.current && spotlightRef.current.parentNode && spotlightRef.current.parentNode.contains(spotlightRef.current)) {
        try {
          spotlightRef.current.parentNode.removeChild(spotlightRef.current);
        } catch (e) {
          // Spotlight already removed, ignore
        }
      }
    };
